function heat_map = filter_3d_heat_map(heat_map_3d, ksstdx, ksstdy, ktstd)
...
l = -ceil(ktstd*size):ceil(ktstd*size);
ht = exp(-(l.^2/(2*ktstd^2)));
ht = ht/sum(ht(:));
ht = reshape(ht, [1, 1, length(ht)]);

heat_map = convn(heat_map_3d, hy, 'same');
heat_map = convn(heat_map, hx, 'same');
heat_map = convn(heat_map, ht, 'same');
